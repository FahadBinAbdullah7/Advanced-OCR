...
const TabButton: React.FC<{ name: string, count?: number, isActive: boolean, onClick: () => void, disabled?: boolean }> = ({ name, count, isActive, onClick, disabled }) => (
    <button onClick={onClick} disabled={disabled} className={`px-3 py-1.5 text-sm font-semibold transition-colors mr-2 relative ${ isActive ? 'text-cyan-400 border-b-2 border-cyan-400' : 'text-gray-400 hover:text-white border-b-2 border-transparent'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
...
export const RightPanel: React.FC<RightPanelProps> = ({ extraction, extractions, onSelectExtraction, onPerformQAC, isQACProcessing, onImageAction, onColorizeToggle }) => {
    ...
    const textToDisplay = activeTab === 'qac' && extraction?.qacText ? extraction.qacText : extraction?.text;
                        ...
                        <div className="flex-grow overflow-hidden mt-2 flex flex-col">
                             ...
                             {activeTab === 'text' && (
                                <div className="flex flex-col h-full">
                                    <textarea value={extraction.text} readOnly className="flex-grow bg-gray-900/50 rounded-lg border border-gray-700 p-3 text-sm font-mono whitespace-pre-wrap w-full min-h-[250px] resize-none focus:outline-none focus:ring-2 focus:ring-cyan-500" placeholder="Extracted text will appear here..."/>
                                    <button onClick={onPerformQAC} disabled={isQACProcessing} className="w-full mt-3 flex items-center justify-center gap-2 bg-gray-700 hover:bg-gray-600 disabled:opacity-50 text-white font-semibold py-2 px-4 rounded-lg transition-colors">
                                        {isQACProcessing ? <><Loader2Icon className="w-5 h-5"/> Processing...</> : <><Wand2Icon className="w-5 h-5 text-cyan-400" /> Advanced QAC & Math</>}
                                    </button>
                                </div>
                             )}
                             {/* QAC Tab */}
                             {activeTab === 'qac' && (
                                <div className="h-full flex flex-col">
                                    <textarea value={extraction.qacText} readOnly className="flex-grow bg-gray-900/50 rounded-lg border border-gray-700 p-3 text-sm font-mono whitespace-pre-wrap w-full min-h-[150px] resize-none focus:outline-none focus:ring-2 focus:ring-cyan-500" placeholder="Corrected text will appear here..."/>
                                    <div className="h-[150px] overflow-y-auto pr-2 mt-2 border-t border-gray-700 pt-2">
                                        {extraction.isQACProcessed ? (
                                            extraction.qacFixes && extraction.qacFixes.length > 0 ? (
                                                <table className="w-full text-sm text-left">
                                                    ...
                                                    <tbody>{extraction.qacFixes.map((fix, i) => (<tr key={i} className="border-b border-gray-700 hover:bg-gray-900/50"><td className="px-2 py-1 font-mono text-red-400">{fix.original}</td><td className="px-2 py-1 font-mono text-green-400">{fix.corrected}</td><td className="px-2 py-1">{fix.type}</td></tr>))}</tbody>
                                                </table>
            ...
            <Card>
                ...
                <div className="max-h-48 overflow-y-auto pr-2 space-y-2">
                    {extractions.length > 0 ? extractions.map(ext => (
                        <button key={ext.id} onClick={() => onSelectExtraction(ext)} className={`w-full text-left p-2 rounded-md transition-colors text-sm ${extraction?.id === ext.id ? 'bg-cyan-500/20 text-white' : 'bg-gray-700/50 hover:bg-gray-700 text-gray-300'}`}>
    ...
    return (
        <div className="bg-gray-900/70 p-3 rounded-lg">
            <div className="flex gap-3">
                <div className="flex-shrink-0 w-[100px] h-[100px] bg-gray-800 rounded-md flex items-center justify-center">
                    {image.enhancedImageUrl ? 
                        <img src={image.enhancedImageUrl} alt="Enhanced" className="border border-gray-700 rounded-md shadow-sm object-contain w-full h-full" /> : 
                        <canvas ref={canvasRef} className="border border-gray-700 rounded-md shadow-sm" />
                    }
                </div>
                <div className="flex-grow space-y-2">
                    <p className="text-xs text-gray-400">{image.description || "Detected visual element"}</p>
                    <div className="flex items-center space-x-2">
                        <input type="checkbox" id={`colorize-${image.id}`} checked={!!image.colorize} onChange={e => onColorizeToggle(image.id, e.target.checked)} className="h-4 w-4 rounded bg-gray-700 border-gray-600 text-cyan-500 focus:ring-cyan-600"/>
            ...
            {showBase64 && (
                <textarea readOnly value={image.base64} className="mt-2 w-full h-16 bg-gray-900 text-xs font-mono p-1 rounded-md border border-gray-700 focus:ring-1 focus:ring-cyan-500"/>
            )}
            {image.isProcessing && <p className="text-xs text-cyan-400 mt-1 animate-pulse">Processing image...</p>}
        </div>
    );
};